...
def get_sbj_scan_list(when='orig', return_snycq=True):
    ...
    return_snycq : boolean 
        flag to select whether or not to return the corresponding sncq table
    ...
    snycq_df: pd.dataframe with SNYCQ answers

    Raises
    ----------
    
    AssertionError : `when` keyword is not an acceptable value for processing. 


    """
    assert when in ['orig','post_struct','post_funct','post_motion'], "++ ERROR: Wrong 'when' argument. Please use one of the following: orig, post_struct, post_funct, post_motion"
